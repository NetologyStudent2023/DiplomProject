...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтоНовыйДокумент Тогда
		ТекстСообщения = СтрШаблон("Новый заказ № %1 от %2: специалист %3, дата и время проведения работ - %4 с %5 по %6", ТекущийОбъект.Номер, 
					                ТекущийОбъект.Дата, ТекущийОбъект.Специалист, ТекущийОбъект.ДатаПроведенияРабот, 
					                ТекущийОбъект.ВремяНачалаРабот, ТекущийОбъект.ВремяОкончанияРабот);
	...
	Если ДатаПроведенияРаботВДокументе <> ТекущийОбъект.ДатаПроведенияРабот Тогда
		ТекстДатаРабот = СтрШаблон("дата проведения работ - %1, ", ТекущийОбъект.ДатаПроведенияРабот);
		ЕстьИзмененияВЗаказе = Истина;
	Иначе
	...
	Если ВремяНачалаРаботВДокументе <> ТекущийОбъект.ВремяНачалаРабот Тогда
		ТекстВремяНачалаРабот = СтрШаблон("время начала проведения работ - %1, ", ТекущийОбъект.ВремяНачалаРабот);
		ЕстьИзмененияВЗаказе = Истина;
	Иначе
	...
	Если ВремяОкончанияРаботВДокументе <> ТекущийОбъект.ВремяОкончанияРабот Тогда
		ТекстВремяОкончанияРабот = СтрШаблон("время окончания проведения работ - %1", ТекущийОбъект.ВремяОкончанияРабот);
		ЕстьИзмененияВЗаказе = Истина;
	Иначе
	...
	Если ЕстьИзмененияВЗаказе Тогда
		ТекстСообщения = СтрШаблон("Заказ № %1 от %2 изменен: ", Номер, Дата) + ТекстСпециалист + ТекстДатаРабот + ТекстВремяНачалаРабот + ТекстВремяОкончанияРабот;
		НовоеУведомление = Справочники.ВКМ_УведомленияТелеграмБоту.СоздатьЭлемент();
		НовоеУведомление.Текст = ТекстСообщения;
		НовоеУведомление.Записать(); 
	КонецЕсли;
